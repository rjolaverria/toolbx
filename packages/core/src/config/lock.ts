 ...
 * no POSIX `flock`). A crashed holder cannot deadlock subsequent commands: a lock
 * is stolen when its recorded owner pid is gone (same host) or its age exceeds
 * `staleMs`.
 *
...
export async function withConfigLock<T>(
  ...
  await acquire(lockDir, timeoutMs, staleMs, pollMs);
...
async function writeMeta(lockDir: string): Promise<void> {
  const meta: LockMeta = { pid: process.pid, host: hostname(), ts: Date.now() };
  await fs
...
async function isStale(lockDir: string, staleMs: number): Promise<boolean> {
  ...
  if (meta && typeof meta.ts === 'number') {
    ...
    // A live pid number on another host must not be probed against the local
    // process table, so only steal a dead-pid lock when the host matches.
    if (meta.host === hostname() && typeof meta.pid === 'number' && !isAlive(meta.pid)) {
      return true;
    }
    return false;
  }
