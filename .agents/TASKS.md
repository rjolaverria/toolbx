...
- [ ] **P3-09** — [Route gateway/daemon OAuth refresh through the credential lock](tasks/P3-09-gateway-refresh-credential-lock.md) — the long-lived gateway refreshes tokens without `withCredentialLock`, so it can still race `auth logout`; surfaced during P3-08 review (separate subsystem from the CLI mutation paths)
