...
Phase 3 is complete: every task below (P3-01 through P3-05) is closed, and the
SPECS §6.7 acceptance criteria 1–10 hold end to end. Imported, enabled custom
tools are exposed through the gateway's `tools/list`/`tools/call` and the
daemon-backed `tlbx run` path alongside proxied upstream tools, exercised by the
P3-05 gateway and CLI integration suites. (P3-06 OS-level sandbox and P3-07
concurrency-safe persistence remain tracked below as follow-ups beyond the §6.7
criteria.)
...
- [x] **P3-08** — [Per-server-name serialization of credential (token-store) mutations](tasks/P3-08-credential-mutation-serialization.md) — closed by this branch; a shared per-name `withCredentialLock` (keyed by `sha256(serverName)`) serializes every token-store mutation across `add-http` OAuth, `auth login | logout | refresh`, and `doctor --fix`, replacing the global OAuth-registration lock
- [x] **P3-09** — [Route gateway/daemon OAuth refresh through the credential lock](tasks/P3-09-gateway-refresh-credential-lock.md) — closed by this branch; the OAuth provider's `saveTokens` read-modify-write now runs under the per-server-name `withCredentialLock` (rooted at the runtime's `configDir`, threaded provider→http→session→runtime), and a refresh-grant save that finds the record gone aborts via `CredentialRemovedDuringRefreshError` (classified as `auth_required`) instead of resurrecting the credential, so a long-lived gateway refresh can no longer race `tlbx auth logout`
- [x] **P3-10** — [Align the credential-lock domain with the token-store key namespace](tasks/P3-10-credential-lock-domain.md) — closed by this branch; `resolveCredentialLockRoot(config.auth.storage)` roots the credential lock at the keychain's per-user domain (`~/.toolbox/credential-locks/dev.toolbox.cli`, anchored at the account-database home so it does not vary with `TMPDIR` across a daemon vs. a terminal CLI; env-overridable via `TOOLBOX_CREDENTIAL_LOCK_DIR` for tests) instead of the per-invocation config dir, so the CLI credential commands, `doctor --fix`, and the gateway refresh all serialize on one lock per stored credential regardless of the `-c` config used
- [x] **P3-11** — [Investigate rare steal-path mutual-exclusion flake under load](tasks/P3-11-lock-steal-mutual-exclusion-flake.md) — closed by this branch; root-caused to a real steal-path TOCTOU exposed by libuv's threadpool (the fs primitives all hold on Linux ext4/tmpfs/overlayfs) — `stealStale` removed a _replaceable_ (empty/absent) slot with an unconditional rename-aside, so a fresh holder that published into the slot between the re-confirm and the rename was renamed away, admitting a second critical section. `evaluateStale` now returns a removal strategy and the steal removes a meta-bearing (non-empty, stable) lock by rename-aside but an empty/absent slot only with an empty-only `rmdir` that no-ops once a holder has published. Two deterministic regression tests (absent slot + empty stale dir) drive the race through an `afterReconfirm` seam (red before the fix), and 3600 Linux tmpfs/overlayfs stress rounds under `UV_THREADPOOL_SIZE=8` (meta and empty plants) go from 4–12% violations to zero. Also deflaked two sibling timing tests (condition waits instead of fixed head-start sleeps)
