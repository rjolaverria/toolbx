...
- Per-package `vitest.config.ts` — set `coverage.thresholds.{lines, statements, branches, functions}` to the floor of (current value − 2). The slack absorbs noise; a real regression of more than two points fails the gate.
