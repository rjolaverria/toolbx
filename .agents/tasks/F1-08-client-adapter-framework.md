...
- **`packages/core/src/clients/claude.ts`** — Claude Code adapter:
  ...
  - `install()`:
    ...
    7. Otherwise: write the merged content to `<configPath>.tmp.<pid>`, fsync, back up the original to `<configPath>.bak.<ISO-8601>`, then `rename` the tmp file over the original (atomic on POSIX). Before the rename, re-stat the original and compare `mtime`+size to the values captured in step 1; if they differ, abort with `{ ok: false, reason: 'Claude Code modified ~/.claude.json while we were merging', hint: 're-run `tlbx client install claude`' }` and leave the original untouched.
