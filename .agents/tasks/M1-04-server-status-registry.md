...
**README references**: §2.5, §4.8 (criterion 11), §7 (Milestone 1)

## Goal

A single in-memory registry that tracks one `ServerStatus` per configured upstream server, plus the metadata listed in README §2.5 (transport type, enabled state, tool count, last connected, last error, recent log lines). The registry is the source of truth for `tlbx status` and any future UI.
