...
# that the four @toolbox/* packages publish together, that pnpm rewrites their
# workspace:^ refs to real versions, that `npx @toolbox/cli` resolves the whole
...
# Anonymous publish + read for the @toolbox scope; everything else proxies npmjs.
cat > "$CONFIG" <<YAML
storage: $STORAGE
uplinks:
  npmjs:
...
echo "▶ building and publishing all @toolbox packages to the local registry"
...
echo "  ✓ published @toolbox/core, custom-tools, mcp-gateway, cli @ 0.1.0"

echo "▶ clean global install of @toolbox/cli from the local registry"
export NPM_CONFIG_PREFIX="$PREFIX"
export TOOLBOX_CONFIG="$WORK/config.json"
export PATH="$PREFIX/bin:$PATH"
npm install -g --registry "$REGISTRY" @toolbox/cli@0.1.0
...
tlbx stop >/dev/null 2>&1 || true
echo "✓ PUBLISHED SHAPE VERIFIED — npx @toolbox/cli works end-to-end from a clean install"
