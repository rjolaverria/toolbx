#!/usr/bin/env bash
# Clean-install smoke test for the published @toolbx/cli tarball.
#
...
#
# Usage:
#   scripts/verify-tarball.sh path/to/toolbx-cli-0.1.0.tgz
#   scripts/verify-tarball.sh --from-npm @toolbx/cli@0.1.0
#
# Env:
...
if [[ "$SPEC" == "--from-npm" ]]; then
  FROM_NPM=1
  SPEC="${2:?--from-npm requires a package spec, e.g. @toolbx/cli@0.1.0}"
fi
